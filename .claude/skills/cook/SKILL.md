...
**Example:**
```
/bs:cook "Add user authentication to the app" --fast
/bs:cook path/to/plan.md --auto
...
6. **Revit-specific scout (MANDATORY khi detect):** Nếu thấy `.csproj` chứa `Sdk="Nice3point.Revit.Sdk"` HOẶC file `.addin` HOẶC reference `Autodesk.Revit.DB`:
   - Đọc `.csproj` lấy: `<Configurations>` (R22..R27?), `<EnableDynamicLoading>`, `<IsRepackable>`, `<UseWPF>`, list `<PackageReference>` (Nice3point.Toolkit, CommunityToolkit.Mvvm, Serilog version).
   - Detect DI mode: search `Host.cs` / `HostingConfiguration.cs` / `services.Add*` để biết `disabled`/`container`/`hosting`.
   - Detect Theme: kiểm tra `Resources/Themes/Theme.xaml` có chưa? Nếu chưa → activate skill `revit-xaml-styles`.
   - Detect test framework: search `Tests/*.csproj` xem TUnit/xUnit/RevitTest.
   - Detect logging: search `LoggerConfiguration.cs` xem Serilog setup.
...
<HARD-GATE-BUILD-VERIFY>
**Áp dụng cho Revit Add-In project (Nice3point SDK).**

Sau khi implement code, TRƯỚC khi spawn `tester` subagent:

1. **MUST run `dotnet build -c Debug.R<active-version>`** — với `<active-version>` là Revit version dev hiện tại (mặc định R27).
2. Nếu build fail → STOP. KHÔNG được skip. Activate skill `revit-debug` → đọc error → fix → rebuild.
3. Nếu multi-version code (`#if REVIT...`) → build thêm config khác để verify preprocessor:
   ```bash
   dotnet build -c Debug.R22  # version cũ nhất
   dotnet build -c Debug.R27  # version mới nhất
   ```
4. Build OK mới chuyển sang test step.

User override: chỉ skip khi user explicit nói "đừng build" hoặc dùng `--no-build` flag tự thêm.
</HARD-GATE-BUILD-VERIFY>
...
### Revit Add-In specific anti-rationalization

| Thought | Reality |
|---------|---------|
| "Inline style XAML cho nhanh" | Phá Theme.xaml → debt. Dùng `{DynamicResource ...}` |
| "Set DataContext trong XAML cho gọn" | Phá DI → ViewModel không inject được ILogger/Service. Set qua container trong code-behind |
| "Tự viết RelayCommand cho 1 button" | CommunityToolkit.Mvvm đã có `[RelayCommand]` → zero boilerplate |
| "Bỏ `<IsRepackable>true</IsRepackable>` cho build nhanh hơn" | Mất ILRepack → DLL conflict với add-in khác trong production |
| "Hardcode `#1E1E1E` cho dark background nhanh" | Không swap được Light theme. Dùng `{DynamicResource Brush.Background}` |
| "Skip `dotnet build` vì IDE đã build rồi" | IDE build có thể stale. CLI build verify clean state |
| "Gọi `doc.NewTransaction` từ ViewModel" | Modeless UI crash. Phải qua `ExternalEvent` |
| "Test pure logic = mock Document" | `Document` sealed, không mock được. Tách layer pure logic riêng cho xUnit |
...
**Claude Tasks:** Utilize `TaskCreate`, `TaskUpdate`, `TaskGet`, `TaskList` during implementation step. **Fallbabs:** These are CLI-only tools — unavailable in VSCode extension. If they error, use `TodoWrite` for progress tracking instead.
...
- **Finalize (MANDATORY - never skip):**
  1. **Activate `/bs:project-management` skill (MANDATORY)** → run full plan sync-back across ALL `phase-XX-*.md` (not only current phase), update `plan.md` status/progress, hydrate Claude Tasks, generate progress report
  ...
  5. Run `/bs:journal` to write a concise technical journal entry upon completion

## Required Subagents (MANDATORY)

| Phase | Subagent / Skill | Requirement |
|-------|----------|-------------|
| Research | `researcher` | Optional in fast/code |
| Scout | `bs:scout` | Optional in code |
| Plan | `planner` | Optional in code |
| UI Work | `ui-ux-designer` | If frontend work |
| **Revit scaffold** | activate skill `revit-addin` | **MUST** khi tạo project mới Nice3point |
| **Revit XAML/MVVM** | activate skills `revit-wpf-mvvm` + `revit-xaml-styles` | **MUST** khi chạm `.xaml` hoặc `*ViewModel.cs` |
| **Revit build verify** | `dotnet build -c Debug.R<XX>` | **MUST** sau implement (xem HARD-GATE-BUILD-VERIFY) |
| **Revit test framework picker** | activate skill `revit-test` | **MUST** khi viết test cho add-in |
| **Revit runtime troubleshoot** | activate skill `revit-debug` | **MUST** khi build/load/runtime fail |
| Testing | `tester`, `debugger` | **MUST** spawn |
| Review | `code-reviewer` | **MUST** spawn |
| Finalize | `/bs:project-management` skill + `docs-manager`, `git-manager` subagents | **MUST** invoke all |
...
- Revit project: nếu user tạo PR mà chưa run `dotnet build -c Debug.R<active-version>` thì block PR.

## Revit Smoke Test (Optional, Recommended)

Sau khi test pass + code review pass, trước khi finalize:

1. Use `AskUserQuestion`: "Build pass + tests pass. Muốn F5 launch Revit để smoke test UI thật không? [Yes / No, finalize]"
2. Nếu Yes: hướng dẫn user mở Rider/VS, chọn `Debug.R<XX>` config, nhấn F5. Activate skill `revit-debug` nếu có issue.
3. Nếu No: tiếp tục finalize.

Smoke test catch: visual artifact (sai theme), modal-owner sai, ribbon button placement, icon size, layout responsive.
...
**Typically follows:** `/bs:plan` (execute a plan), `/bs:brainstorm` (implement agreed solution)
**Typically precedes:** `/bs:code-review` (review after implementation), `/bs:test` (validate changes)
**Related:** `/bs:fix` (alternative for bug fixes), `/bs:plan` (create plan before cooking)
