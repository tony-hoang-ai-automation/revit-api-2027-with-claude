...
# bs:scenario — Edge Case & Scenario Explorer
...
| Next Step | Skill | How |
|-----------|-------|-----|
| Generate test cases from scenarios | `bs:test` | Pass scenario table as input context |
| Inform implementation plan risks | `bs:plan` | Paste Critical/High rows into risk assessment |
| Deep persona debate on top risks | `bs:predict` | Feed Critical scenarios as the change proposal |
...
/bs:scenario src/api/payment.ts
/bs:scenario "User registration with OAuth providers"

# Bounded iterative — exactly 25 iterations
/bs:scenario src/api/payment.ts --iterations 25

# Saturation — stop when coverage exhausted
/bs:scenario "Add multi-tenancy to the database layer" --saturation

# Saturation with domain hint for priority dimension ordering
/bs:scenario src/middleware/auth.ts --saturation --domain security
```
