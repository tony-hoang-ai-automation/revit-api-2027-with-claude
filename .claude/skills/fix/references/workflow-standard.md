...
2. Activate `bs:debug` skill. Use `debugger` subagent if needed.
3. Activate `bs:sequential-thinking` — form hypotheses through structured reasoning.
...
- Apply `bs:problem-solving` skill if stuck
- Use `bs:sequential-thinking` for complex logic
...
- Activate `bs:project-management` for task sync-back and plan status updates
...
| 2 | `bs:debug`, `bs:sequential-thinking`, `debugger` subagent, parallel `Explore`, (`bs:problem-solving` auto) |
| 3 | `bs:problem-solving` (if stuck), `bs:sequential-thinking` (complex logic) |
| 4 | `tester` subagent, parallel `Bash` verification |
| 5 | `code-reviewer` subagent |
| 6 | `bs:project-management`, `git-manager`, `docs-manager` subagents |
