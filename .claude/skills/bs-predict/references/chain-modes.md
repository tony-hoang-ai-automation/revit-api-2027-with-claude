...
These chain modes absorb upstream `/autoresearch:reason` and `/autoresearch:probe` (see `uditgoenka/autoresearch`, MIT) — workflows that always chain off `/autoresearch:predict` and never run independently. Local design folds them into bs:predict rather than shipping standalone skills.
...
- The decision is mechanically determinable (e.g. perf benchmark, security audit) — use `/bs:loop` or `/bs:security` instead

### Safety inheritance

Every refinement round commits to git as an atomic checkpoint, named `predict-chain-reason-round-N`. If a round produces a worse candidate (judge score regresses by >2), the round is reverted, not kept. Same Modify → Verify → Keep/Discard discipline as `/bs:loop`.
...
### Coupling with `/bs:plan`

Probe output is shaped to feed directly into `/bs:plan` — each constraint card maps to a plan acceptance criterion. Recommended sequence: `/bs:predict ... --chain probe` → `/bs:plan` (passes the constraint table as input).
