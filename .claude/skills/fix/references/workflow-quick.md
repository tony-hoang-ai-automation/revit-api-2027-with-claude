...
Activate `bs:debug` skill. Activate `bs:sequential-thinking` for structured analysis.
...
1. **Activate `/bs:project-management` skill (MANDATORY)** → sync plan/task status if fix is part of a plan, update progress, hydrate Claude Tasks.
...
5. Run `/bs:journal` to log decisions.
...
|------|------------------|
| 1 | `bs:scout` (minimal) or direct file read |
| 2 | `bs:debug`, `bs:sequential-thinking` |
| 3 | Parallel `Bash` for verification |
| 4 | `code-reviewer` subagent |
| 5 | Report |
| 6 | `/bs:project-management` (MANDATORY), `docs-manager`, `git-manager`, `/bs:journal` |

**Extra:** `bs:context-engineering` if dealing with AI/LLM code
...
- Step 6 finalize is MANDATORY for every fix — `/bs:project-management` is NOT optional
