...
|------------|-----------|---------|
| `/bs:docs init` | `references/init-workflow.md` | Analyze codebase and create initial documentation |
| `/bs:docs update` | `references/update-workflow.md` | Analyze codebase and update existing documentation |
| `/bs:docs summarize` | `references/summarize-workflow.md` | Quick analysis and update of codebase summary |
...
When authoring or refreshing diagrams in `system-architecture.md`, apply the universal SVG layout rules from `/bs:tech-graph`'s `references/svg-layout-best-practices.md` (component spacing, arrow routing, label placement, z-index ordering). Pair with `/bs:preview --diagram` for visual self-review, or use `/bs:tech-graph` directly for publish-grade output.
