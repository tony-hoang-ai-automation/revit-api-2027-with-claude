...
1. `bs:ui-ux-pro-max` - Design database (ALWAYS FIRST)
2. `bs:ui-ux-pro-max` - Design principles
3. `bs:frontend-design` - Implementation patterns
...
Analyze screenshots/videos with `bs:ai-multimodal` skill.
...
Screenshot + `bs:ai-multimodal` analysis.
...
Use `bs:agent-browser`, Chrome MCP / `chrome-devtools-mcp`, or project-native browser tests.
...
## Tips
- Use `bs:ai-multimodal` for generating visual assets
