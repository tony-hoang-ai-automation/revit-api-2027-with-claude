...
Scans SKILL.md bodies for /bs: references, builds a directed graph,
...
# Match /bs:skill-name but NOT inside code fences
CODE_FENCE_RE = re.compile(r"```[\s\S]*?```", re.MULTILINE)
SKILL_REF_RE = re.compile(r"/bs:([a-z0-9][\w-]*)")
...
def _extract_body_refs(content: str, own_name: str) -> set[str]:
    """Extract /bs: skill references from body text, excluding code fences."""
    m = FRONTMATTER_RE.match(content)
    ...
    # The regex captures what's after /bs: so "plan" from "/bs:plan"
    refs.discard(own_name)
    # Also discard dir-name form of own name (e.g. "ck-plan" vs "plan")
    refs.discard(own_name.replace("bs:", ""))
    return refs
...
def _run_self_tests():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        fixtures = {
            "cook": '---\nname: bs:cook\ndescription: "Cook things"\ncategory: utilities\nkeywords: [impl]\n---\n# Cook\nAfter planning, run /bs:scout then /bs:code-review.\n',
            "plan": '---\nname: bs:plan\ndescription: "Plan things"\ncategory: utilities\nkeywords: [plan]\n---\n# Plan\nThis skill creates plans.\n',
            "scout": '---\nname: bs:scout\ndescription: "Scout"\ncategory: dev-tools\nkeywords: [scout]\nrequires: [ck:scout]\n---\n# Scout\nExplore code. See /bs:cook for next step.\n```\n/bs:plan should not match inside fence\n```\n',
            "orphan-skill": '---\nname: bs:orphan\ndescription: "Orphan"\ncategory: other\nkeywords: []\n---\n# Orphan\nNo refs here.\n',
            "broken-ref": '---\nname: bs:broken\ndescription: "Broken"\ncategory: other\nkeywords: []\n---\n# Broken\nSee /bs:nonexistent for help.\nAlso /bs:cook is good.\n',
        }
        ...
        print("Self-tests:")
        # T1: Detects /bs:scout in cook's body
        _assert("scout" in graph["edges"].get("cook", []), "T1: Detects /bs:scout ref in body")
        # T2: cook→code-review edge exists
        _assert("code-review" in skills["cook"]["body_refs"] or "code-review" in graph["edges"].get("cook", []),
                "T2: Detects /bs:code-review ref in body")
        ...
        _assert(len(broken_refs) > 0, "T6: Broken reference /bs:nonexistent detected")
        ...
        # T8: Code fence exclusion (scout has /bs:plan in code fence — should NOT count)
