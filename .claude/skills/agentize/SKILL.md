...
- **Companion skill** — a `/bs:*` skill discoverable on the Claude Plugins Marketplace
...
Scope: converting existing code into CLI and/or MCP. Not for: building a server from scratch (use `/bs:mcp-builder`), raw npm scaffolding, or publishing without an agent-use story.

## Usage

```text
/bs:agentize [feature-or-module] [--both|--mcp|--cli] [--auto|--ask]
```
...
Invoke `/bs:project-management` **before** touching code. This skill owns plan/task lifecycle; `agentize` is a consumer.
...
Invoke `/bs:scout` to understand the target codebase. Without this, everything downstream is guessed.
...
1. **Tests** — invoke `/bs:test` to generate:
...
3. **Docs** — invoke `/bs:docs` to generate:
...
4. **Companion skill** — invoke `/bs:skill-creator` to generate:
...
Next: /bs:cook <plan-path> to execute any remaining implementation.
```
