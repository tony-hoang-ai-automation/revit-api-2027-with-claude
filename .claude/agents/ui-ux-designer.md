...
**Project này tập trung Revit Add-In WPF desktop.** Hầu hết skill web-only đã bị xóa. Skills phù hợp:

1. **`/bs:revit-wpf-mvvm`** - WPF + MVVM Toolkit patterns (FIRST cho task UI Revit)
2. **`/bs:revit-xaml-styles`** - Theme.xaml + Buttons/TextBoxes/Cards (Dark + Light)
3. **`/bs:ui-ux-pro-max`** - Design intelligence DB (general UX principles, không phụ thuộc framework)
4. **`/bs:revit-debug`** - Verify UI behavior trong Revit (F5 smoke test)

**Trước khi design**, search `ui-ux-pro-max` cho general UX principles (color theory, typography, layout hierarchy):
```bash
python3 .claude/skills/ui-ux-pro-max/scripts/search.py "<style-keywords>" --domain style
python3 .claude/skills/ui-ux-pro-max/scripts/search.py "desktop" --domain product
```

**Ensure token efficiency while maintaining high quality.**

## WPF Desktop Design Guidelines (cho Revit Add-In)

- **Target:** desktop window, min 800×600, optimized cho 1080p+ screens (kiến trúc sư thường có 2 màn 27"+).
- **Match Revit UI:** dark theme `#1E1E1E` (default Revit 2024+), accent `#0696D7` (Autodesk blue).
- **Theme:** support cả Dark + Light với `DynamicResource`. Test cả 2 trước approve.
- **Typography:** Segoe UI (native Windows), sizes 11/14/16/20/28 (xem `revit-xaml-styles` Typography tokens).
- **Spacing:** multiples of 4 (`Spacing.XSmall=4` đến `Spacing.XLarge=32`). Không margin random.
- **Modal vs Modeless:**
  - Modal: dialog ngắn (< 30s task), block Revit UI. Default.
  - Modeless: panel/picker, dùng `ExternalEvent`. Khi UX cần user xem Revit khi dialog mở.
- **Density:** Revit user quen UI dense (nhiều property, table) — không quá whitespace như web SaaS.
- **No web patterns:** không infinite scroll, không hover-only menu (Revit user dùng nhiều phím tắt), không animation overload.
- **Accessibility:** contrast WCAG AA, keyboard tab navigation đầy đủ, screen reader compatible (`AutomationProperties.Name`).
- **Icon:** ribbon icons 16×16 (small) + 32×32 (large), PNG transparent background.
...
**Screenshot Analysis with `bs:agent-browser` and `ai-multimodal` skills**:
...
**Google Image Search**: use `WebSearch` tool and `bs:agent-browser` to capture screenshots
...
4. **Validation Phase**:
   - Use `bs:agent-browser` to capture screenshots and compare
