...
- Use `bs:docs-seeker` skill to read latest documentation of external plugins/packages
- Leverage `bs:ai-multimodal` skill to analyze visual materials and mockups
- Query `psql` command to understand current database structure and existing data
- Employ `bs:sequential-thinking` skill for complex problem-solving that requires structured analysis
...
2. Existing modules/files relevant to the user's topic (use `bs:scout` or Glob/Grep)
...
flowchart TD
    ...
    H --> I{Create Plan?}
    I -->|Yes| J[Pick /bs:plan mode<br/>--tdd or default]
    I -->|No| K[End Session]
    J --> L[Journal]
    K --> L
```

**This diagram is the authoritative workflow.** If prose conflicts with this flow, follow the diagram. The terminal state is either `/bs:plan` or end.

## Your Process
1. **Scout Phase (MANDATORY FIRST STEP)**: Always run before anything else.
   - Use `bs:scout` skill (or Glob/Grep directly for small repos) to map files relevant to the user's topic
...
9. **Finalize Phase (Plan Handoff)**: Once the user has confirmed the proposal AND has no further questions (i.e. brainstorm is converging to close), use `AskUserQuestion` to offer the appropriate `/bs:plan` mode. Pass the brainstorm summary path as context to `/bs:plan` for continuity.
   ...
   **Plan mode selection — present these as options:**
   ...
   |--------|----------------|-----|
   | `/bs:plan --tdd` | Solution refactors existing behavior, modifies critical business logic, or has strong existing test coverage to preserve | Forces tests-first per phase so current behavior is locked in before changes |
   | `/bs:plan` (default) | Standard new feature or moderate change | Produces the standard phase-by-phase implementation plan |
   ...
   **Note:** `/bs:plan validate` and `/bs:plan red-team` are post-plan gates — do NOT offer them here. They are surfaced by `/bs:plan` itself after the plan is produced.
...
10. **Journal Phase**: Run `/bs:journal` to write a concise technical journal entry upon completion.
...
## Output Requirements
**IMPORTANT:** Invoke "/bs:project-organization" skill to organize the reports.
...
**Typically follows:** `/bs:debug` (brainstorm solutions for diagnosed issues), `/bs:scout` (brainstorm after discovery)
**Typically precedes:** `/bs:plan` (plan the agreed solution)
**Related:** `/bs:plan` (plan after brainstorming), `/bs:debug` (debug before brainstorming)
