...
2. Use `ui-ux-designer` + `researcher` subagents in parallel:
   ...
   - Describe assets for `bs:ai-multimodal` skill generation
3. `ui-ux-designer` creates:
   - Design guidelines at `./docs/design-guidelines.md`
   - Wireframes in HTML at `./docs/wireframe/`
4. If no logo provided: generate with `bs:ai-multimodal` skill
5. Screenshot wireframes with `bs:agent-browser` → save to `./docs/wireframes/`
...
Activate **bs:plan** skill: `/bs:plan --hard <requirements>`
...
Activate **bs:cook** skill: `/bs:cook <plan-path>` (interactive mode — review gates at each step)
