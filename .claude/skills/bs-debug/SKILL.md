...
Track investigation pipelines via Claude Native Tasks (TaskCreate, TaskUpdate, TaskList). Hydration pattern for multi-step investigations with dependency chains and parallel evidence collection. **Fallbabs:** Task tools are CLI-only — if unavailable (VSCode extension), use `TodoWrite` for tracking. Debug workflow remains fully functional.
...
Visual verification of frontend implementations via Chrome MCP / `chrome-devtools-mcp` or `bs:agent-browser`. Detect if frontend-related → check browser tool availability → screenshot + console error check → report. Skip if not frontend.
...
- **Codebase:** `bs:docs-seeker` skill for package/plugin docs; `bs:repomix` skill for codebase summary
- **Scouting:** `/bs:scout` or `/bs:scout ext` for finding relevant files
- **Frontend:** Chrome MCP / `chrome-devtools-mcp`, `bs:agent-browser`, or project-native browser tests for visual verification
- **Skills:** Activate `bs:problem-solving` skill when stuck on complex issues
...
**Typically follows:** `/bs:scout` (after locating relevant code)
**Typically precedes:** `/bs:fix` (fix the diagnosed issue), `/bs:brainstorm` (explore solutions for complex problems)
**Related:** `/bs:scout` (discover before debugging), `/bs:fix` (fix after diagnosing)
