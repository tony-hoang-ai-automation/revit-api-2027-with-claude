...
3. **Randomly invoke ONE of:** `/bs:ui-ux-pro-max` OR `/bs:frontend-design` — for layout, hierarchy, visual balance. Pick one at random each run for design variety.
...
Use Chrome headless, `bs:agent-browser`, or Playwright/Puppeteer to capture exact-size screenshots.
...
Invoke `/bs:agent-browser` with instructions to:
...
Use Chrome MCP / `chrome-devtools-mcp`, `bs:agent-browser`, or Playwright to visually inspect each exported PNG:
