...
# bs:security — Security Audit

Runs a structured STRIDE + OWASP security audit on a given scope. Produces a severity-ranked findings report. With `--fix`, applies fixes iteratively using the bs:autoresearch guard pattern.
...
|------|-----------|----------|
| Audit only | `/bs:security <scope>` | Scan → categorize → report (one-shot) |
| Red-team discovery | `/bs:security <scope> --red-team` | Iterate 4 attacker personas → STRIDE/OWASP sweep → report |
| Bounded red-team | `/bs:security <scope> --red-team --iterations N` | Cap persona discovery to N iterations total |
| Audit + Fix | `/bs:security <scope> --fix` | Scan → categorize → fix iteratively |
| Red-team + Fix | `/bs:security <scope> --red-team --fix` | Full persona discovery → fix confirmed Critical/High |
| Bounded fix | `/bs:security <scope> --fix --iterations N` | Limit fix iterations to N |
...
### 4. Dependency Audit
Run the appropriate package audit tool for the detected stabs:
- Node.js: `npm audit`
- Python: `pip-audit`
- Go: `govulncheck`
- Ruby: `bundle audit`
...
4. Uses `bs:autoresearch` guard pattern for regression prevention
...
- Run after `bs:predict` when the security persona flags concerns
- Feed Critical/High findings into `bs:autoresearch --fix` for automated remediation
- Use `bs:scenario` with `--focus authorization` for deeper auth flow testing
- Pair with `bs:plan` to schedule Medium/Low findings as sprint tasks
...
See `/bs:autoresearch` for the full family map.
