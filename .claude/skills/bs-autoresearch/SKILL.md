...
# bs:autoresearch — Autoresearch Family Router
...
The upstream framework ships 11 sub-commands. Locally, 6 are absorbed (4 standalone skills + 2 folded as chain modes) and 5 are not yet absorbed. **For the core loop, use `/bs:loop`.**

| Upstream sub-command | Local skill | Status | Use when... |
|---|---|---|---|
| `/autoresearch` (core) | `/bs:loop` | Faithful | Improving a measurable metric (coverage, bundle size, perf) over N bounded iterations |
| `/autoresearch:predict` | `/bs:predict` | Faithful (in scope) | Multi-persona debate before risky changes; supports `--chain reason` and `--chain probe` (closed in #728) |
| `/autoresearch:scenario` | `/bs:scenario` | Faithful (in scope) | Edge-case generation across 12 dimensions; supports both one-shot and iterative saturation (closed in #729) |
| `/autoresearch:security` | `/bs:security` | Faithful (in scope) | STRIDE + OWASP audit with `--fix`; supports both one-shot and red-team-personas iterative discovery (closed in #730) |
| `/autoresearch:reason` | `/bs:predict --chain reason` | Folded | Subjective refinement loop — folded into `/bs:predict` chain modes rather than shipped as a standalone skill (closed in #728) |
| `/autoresearch:probe` | `/bs:predict --chain probe` | Folded | Requirement interrogation — folded into `/bs:predict` chain modes rather than shipped as a standalone skill (closed in #728) |
| `/autoresearch:plan` | — | Missing | Backfill candidate (HIGH priority) |
| `/autoresearch:debug` | — | Missing | Backfill candidate (HIGH priority) |
| `/autoresearch:fix` | — | Missing | Backfill candidate (MEDIUM); partly covered by `/bs:fix` |
| `/autoresearch:ship` | — | Missing | Backfill candidate (MEDIUM); partly covered by `/bs:ship` |
...
The exception: if you're authoring a new ClaudeKit skill that should adopt the autoresearch pattern (atomic commits, mandatory verify, guarded rollback), read this page + `/bs:loop`'s `references/` for the canonical implementation, then absorb only the relevant sub-command's workflow from upstream.
...
- Core iteration loop: `/bs:loop`
- Pre-implementation debate: `/bs:predict`
- Edge-case generation: `/bs:scenario`
- Security audit (uses autoresearch pattern): `/bs:security`
...
Earlier versions of this skill shipped as a byte-identical copy of `/bs:loop`'s body, marked "deprecated alias." That misrepresented the integration: locally the framework was already split, but the umbrella name absorbed nothing unique. As of 2026-05-02 (epic #711), this page is rewritten as a **concept anchor** so:

- `/bs:security`'s reference to "the bs:autoresearch pattern" lands somewhere meaningful.
