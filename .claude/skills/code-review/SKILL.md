...
| **Checklist review** | Pre-landing, `/bs:ship` pipeline, security audit | `references/checklist-workflow.md` |
...
**Process:**
1. Invoke `/bs:scout` with edge-case-focused prompt
...
**Fallbabs:** Task tools (`TaskCreate`/`TaskUpdate`/`TaskGet`/`TaskList`) are CLI-only — unavailable in VSCode extension. If they error, use `TodoWrite` for tracking and run pipeline sequentially. Review quality is identical.
...
**Typically follows:** `/bs:cook` (review after implementation), `/bs:fix` (review after bug fix)
**Typically precedes:** `/bs:ship` (ship after review passes)
**Related:** `/bs:scout` (scout before reviewing), `/bs:test` (test before reviewing)
