...
**Mandatory:** Activate `bs:scout` skill or launch 2-3 `Explore` subagents in parallel:
```
...
2. Activate `bs:debug` skill (systematic-debugging + root-cause-tracing).
3. Activate `bs:sequential-thinking` — structured hypothesis formation.
4. Spawn parallel `Explore` subagents to test each hypothesis.
5. If 2+ hypotheses fail → auto-activate `bs:problem-solving`.
...
Implement per plan. Use `bs:context-engineering`, `bs:sequential-thinking`, `bs:problem-solving`.
...
- Activate `bs:project-management` for task sync-back, plan status updates, and progress tracking
...
|------|------------------|
| 1 | `bs:scout` OR parallel `Explore` subagents |
| 2 | `bs:debug`, `bs:sequential-thinking`, parallel `Explore`, (`bs:problem-solving` auto) |
| 3 | `researcher` (runs parallel with steps 1+2) |
| 4 | `bs:brainstorm` |
| 5 | `planner` |
| 6 | `bs:problem-solving`, `bs:sequential-thinking`, `bs:context-engineering` |
| 7 | `tester`, parallel `Bash` verification |
| 8 | `code-reviewer` |
| 9 | `bs:project-management`, `docs-manager`, `git-manager` |

**Rules:** Don't skip steps. Validate before proceeding. One phase at a time.
**Frontend:** Use `bs:agent-browser`, Chrome MCP / `chrome-devtools-mcp`, or any relevant project-native browser tests to verify.
**Visual Assets:** Use `bs:ai-multimodal` for visual assets generation, analysis and verification.
