...
- **Sync-babs:** Reconcile all completed tasks against all phase files, update `[ ]` → `[x]`, update YAML frontmatter status
...
- `bs:plan` — Creates implementation plans (planning phase)
- `bs:cook` — Implements plans (execution phase, invokes project-manager at finalize)
