...
# bs:predict — Multi-Persona Pre-Analysis
...
- Trivial or low-risk changes (use `bs:debug` for bugs, `bs:plan` for already-decided tasks)
...
These chain modes absorb upstream `/autoresearch:reason` and `/autoresearch:probe` ([uditgoenka/autoresearch](https://github.com/uditgoenka/autoresearch), MIT). They're folded into bs:predict — not shipped as standalone skills — because they always chain off a predict invocation. See `/bs:autoresearch` for the family map.
...
|---------------|-------|-----|
| Deepen risk scenarios | `bs:scenario` | Feed Risk Summary rows as feature description |
| Create implementation plan | `bs:plan` | Attach Recommendations as constraints to planner |
| High-risk feature implementation | `bs:cook` | Reference CAUTION/STOP items as acceptance gates |
...
```
/bs:predict "Add WebSocket support for real-time notifications"
/bs:predict "Migrate authentication from JWT to session cookies"
/bs:predict "Add multi-tenancy to the database layer"
/bs:predict "Replace REST API with GraphQL" --files src/api/**/*.ts

# Chain modes
/bs:predict "Pick auth library: Passport vs Better Auth" --chain reason
/bs:predict "Move from REST to GraphQL" --chain probe
```
