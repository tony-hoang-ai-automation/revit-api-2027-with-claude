...
related: [bs:repomix, bs:scout, bs:gkg]
maturity: beta
metadata:
  author: safishamsi
...
/bs:scout "auth module"           # find specific files
```
...
**Typically precedes:** `/bs:plan` (understand architecture before planning)
**Related:** `/bs:scout` (quick file search), `/bs:repomix` (full context dump), `/bs:gkg` (semantic symbol navigation)
