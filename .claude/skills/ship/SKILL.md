...
Step 8:  Journal          → Write technical journal via /bs:journal
Step 9:  Docs update      → Update project docs via /bs:docs update (official only)
...
User says `/bs:ship` → run full pipeline → output PR URL.
User says `/bs:ship beta` → ship to dev branch with lighter pipeline.
User says `/bs:ship official` → ship to main with full docs + journal.
...
**Typically follows:** `/bs:code-review` (ship after review passes)
**Typically precedes:** `/bs:journal` (document after shipping)
**Related:** `/bs:code-review` (review before shipping), `/bs:test` (test before shipping)
