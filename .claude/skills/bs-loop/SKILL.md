...
# bs:loop — Autonomous Optimization Loop
...
|-----------|-------------|
| Subjective goals ("make it cleaner") | `bs:cook` |
| Bug fixing with known root cause | `bs:fix` or `bs:debug` |
| One-shot tasks, no repetition needed | `bs:cook` |
| No mechanical metric to measure progress | `bs:cook --interactive` |
...
See `/bs:autoresearch` for the full family map and what's not yet absorbed.
