...
1. Invoke `/bs:journal` skill via `journal-writer` subagent in background:
...
1. Invoke `/bs:docs update` skill via `docs-manager` subagent in background:
...
2. Security chebs: scan staged diff for secrets (API keys, tokens, passwords)
