...
4. **Export** — Screenshot to PNG at exact dimensions via `bs:agent-browser`, Chrome headless, or Playwright
...
4. **Design** — `/ckm:brand` → `/ckm:design-system` → randomly invoke `/bs:ui-ux-pro-max` OR `/bs:frontend-design`; HTML per idea × size
5. **Export** — `bs:agent-browser`, Chrome headless, or Playwright screenshot at exact px (2x deviceScaleFactor)
6. **Verify** — Use Chrome MCP / `chrome-devtools-mcp`, `bs:agent-browser`, or Playwright to visually inspect exported designs; fix layout/styling issues and re-export
