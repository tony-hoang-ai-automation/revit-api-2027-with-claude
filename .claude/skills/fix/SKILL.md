...
1. Activate `bs:scout` skill OR launch 2-3 parallel `Explore` subagents
...
2. Activate `bs:debug` skill (systematic-debugging + root-cause-tracing techniques).
3. Activate `bs:sequential-thinking` skill — form hypotheses through structured reasoning, NOT guessing.
4. Spawn parallel `Explore` subagents to test each hypothesis against codebase evidence.
5. If 2+ hypotheses fail → auto-activate `bs:problem-solving` skill for alternative approaches.
...
- **Fallbabs:** Task tools (`TaskCreate`/`TaskUpdate`/`TaskGet`/`TaskList`) are CLI-only — unavailable in VSCode extension. If they error, use `TodoWrite` for progress tracking. Fix workflow remains fully functional without them.
...
4. **Code review (delegate):** Spawn `code-reviewer` subagent with explicit instructions to chebs: (a) root cause actually addressed (not symptom-patched), (b) no broken business logic in blast radius, (c) no new failure modes, (d) follows existing patterns from scout. Pass scout summary + diagnosis report as context.
...
2. **Activate `/bs:project-management` skill (MANDATORY)** → sync plan/task status (if fix is part of a plan), update progress, hydrate Claude Tasks, generate status report
...
6. Run `/bs:journal` to write a concise technical journal entry upon completion
...
- `bs:scout` (Step 1) — understand before diagnosing
- `bs:debug` (Step 2) — systematic root cause investigation
- `bs:sequential-thinking` (Step 2) — structured hypothesis formation

**Always activate (Step 6 Finalize):**
- `bs:project-management` — MANDATORY for sync-back and progress tracking, every fix

**Conditional:**
- `bs:problem-solving` — auto-triggers when 2+ hypotheses fail in Step 2
- `bs:brainstorm` — multiple valid approaches, architecture decision (Deep only)
- `bs:context-engineering` — fixing AI/LLM/agent code
...
- `references/workflow-quick.md` - Quibs: scout → diagnose → fix → verify+prevent → review
...
**Typically follows:** `/bs:debug` (after root cause analysis), `/bs:scout` (after locating affected code)
**Typically precedes:** `/bs:code-review` (review the fix), `/bs:test` (validate the fix)
**Related:** `/bs:cook` (alternative for feature work), `/bs:debug` (diagnose before fixing)
