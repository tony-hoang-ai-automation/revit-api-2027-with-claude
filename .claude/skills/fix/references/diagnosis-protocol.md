...
Read, don't assume. Use `bs:debug` (systematic-debugging Phase 1).
...
Activate `bs:sequential-thinking` skill. Form hypotheses through structured reasoning.
...
Use `bs:debug` (root-cause-tracing technique). Trace backward:
...
1. Auto-activate `bs:problem-solving` skill
