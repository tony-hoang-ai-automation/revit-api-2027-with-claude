...
4. **Implementation option:** `/bs:cook {absolute-plan-path}/plan.md`
...
2. Read codebase docs; if stale/missing: run `/bs:scout` to search codebase
...
7. **Context reminder:** `/bs:cook {absolute-plan-path}/plan.md`
...
2. Read all relevant docs and use `/bs:scout` across the affected areas
...
8. Output the standard `/bs:cook {absolute-plan-path}/plan.md` reminder
...
7. **Context reminder:** `/bs:cook --parallel {absolute-plan-path}/plan.md`
...
**Invocation:** Run `/bs:plan red-team {plan-directory-path}`.
```
/bs:plan red-team {plan-directory-path}
```
...
| `auto` | Run `/bs:plan validate {plan-directory-path}` |
| `off` | Skip validation |

**Invocation (when prompt mode, user says yes):** Run:
```
/bs:plan validate {plan-directory-path}
```
...
| Mode | Cook Command |
|------|-----------------------------|
| fast | `/bs:cook {path}/plan.md` |
| hard | `/bs:cook {path}/plan.md` |
| deep | `/bs:cook {path}/plan.md` |
| parallel | `/bs:cook --parallel {path}/plan.md` |
| two | `/bs:cook {path}/plan.md` |

If planning ran with `--tdd`, append `--tdd` to the reminder above so cook keeps
the tests-first execution path. Example:
`/bs:cook {path}/plan.md --tdd`
