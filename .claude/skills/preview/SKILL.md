...
### View Mode
- `/bs:preview <file.md>` - View markdown file in novel-reader UI
- `/bs:preview <directory/>` - Browse directory contents
- `/bs:preview --stop` - Stop running server

### Generation Mode (Markdown)
- `/bs:preview --explain <topic>` - Generate visual explanation (ASCII + Mermaid + prose)
- `/bs:preview --slides <topic>` - Generate presentation slides (one concept per slide)
- `/bs:preview --diagram <topic>` - Generate focused diagram (ASCII + Mermaid)
- `/bs:preview --ascii <topic>` - Generate ASCII-only diagram (terminal-friendly)

### Generation Mode (HTML)
- `/bs:preview --html --explain <topic>` - Self-contained HTML explanation
- `/bs:preview --html --slides <topic>` - Magazine-quality HTML slide deck
- `/bs:preview --html --diagram <topic>` - HTML diagram with zoom controls
- `/bs:preview --html --diff [ref]` - Visual diff review
- `/bs:preview --html --plan-review [plan-file]` - Plan vs codebase comparison
...
| Flag without topic | Ask user: "Please provide a topic: `/bs:preview --explain <topic>`" |
...
| Server startup failure | Check if port in use, try `/bs:preview --stop` first |
...
Use `/bs:mermaidjs-v11` skill for Mermaid syntax validation.
...
- For `--slides`: consider invoking `/bs:ui-ux-pro-max` for richer style selection
