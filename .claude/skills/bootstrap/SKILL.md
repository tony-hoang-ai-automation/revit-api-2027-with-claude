...
```
/bs:bootstrap <user-requirements>
```
...
**Example:**
```
/bs:bootstrap "Build a SaaS dashboard with auth" --fast
/bs:bootstrap "E-commerce platform with Stripe" --parallel
```
...
### Planning Phase
Activate **bs:plan** skill with mode-appropriate flag:
- `--full` → `/bs:plan --hard <requirements>` (thorough research + validation)
- `--auto` → `/bs:plan --auto <requirements>` (auto-detect complexity)
- `--fast` → `/bs:plan --fast <requirements>` (skip research)
- `--parallel` → `/bs:plan --parallel <requirements>` (file ownership + dependency graph)

Planning skill outputs a plan path. Pass this to cook.

### Implementation Phase
Activate **bs:cook** skill with the plan path and mode-appropriate flag:
- `--full` → `/bs:cook <plan-path>` (interactive review gates)
- `--auto` → `/bs:cook --auto <plan-path>` (explicit autonomous implementation)
- `--fast` → `/bs:cook <plan-path>` (skip extra research, keep cook review gates)
- `--parallel` → `/bs:cook --parallel <plan-path>` (multi-agent execution)
...
- Run `/bs:journal` to write a concise technical journal entry upon completion
