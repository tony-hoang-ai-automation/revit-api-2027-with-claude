...
## Revit Context Awareness (MANDATORY)

Khi nhận task, kiểm tra:

1. **Cwd có `.csproj` chứa `Sdk="Nice3point.Revit.Sdk"`?** → Project Nice3point detected.
2. **Task description chứa keyword:** `revit`, `add-in`, `addin`, `ribbon`, `wpf`, `xaml`, `mvvm`, `external command`?
3. **Plan dir đã có context Revit không?**

Nếu Revit context → MUST dùng **Stack-Aware 6-Phase Template** (xem `skills/ck-plan/SKILL.md` section "Stack-Aware Planning"):

| Phase | Tên | Mục đích |
|---|---|---|
| 0 | Scaffold | Template Nice3point + DI + WPF + Logging |
| 1 | Multi-version | Configurations + preprocessor |
| 2 | Architecture | Folder structure + DI registration |
| 3 | WPF UI | View↔ViewModel mapping + Theme.xaml |
| 4 | Revit API | Transaction + ExternalEvent + Collectors |
| 5 | Test + Deploy | TUnit/xUnit + Release build |

Activate skills tương ứng trong plan phase files:
- Phase 0–1: link `/bs:revit-addin`
- Phase 3: link `/bs:revit-wpf-mvvm` + `/bs:revit-xaml-styles`
- Phase 5: link `/bs:revit-test`

**Default values cho Revit project** (đã chốt cho repo này):
- Versions: 2022–2027 (6 configs)
- DI mode: `container`
- Theme: Dark + Light với DynamicResource
- Logging: Serilog file
- Manifest: `Application` (mặc định, có Ribbon)

Khi user yêu cầu khác default → confirm qua `AskUserQuestion` trước khi finalize.
