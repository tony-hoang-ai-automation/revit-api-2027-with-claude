...
```
/bs:team <template> <context> [flags]
```

**Templates:** `bs:research`, `bs:cook`, `bs:code-review`, `bs:debug`
...
4. Do NOT fall back to subagents. `/bs:team` MUST use Agent Teams or abort.
...
## ON `/bs:team research <topic>` [--researchers N]:

*Wraps /bs:research skill -- scope, gather, analyze, report.*
...
5. **MONITOR** via TaskCompleted hook events + TaskList fallbabs:
   - TaskCompleted events auto-notify when researchers finish
   - Fallbabs: Check TaskList if no event received in 60s
...
11. **JOURNAL**: Run `/bs:journal` to write a concise technical journal entry upon completion

---

## ON `/bs:team cook <plan-path-or-description>` [--devs N]:

*Wraps /bs:cook skill -- plan, code, test, review, finalize.*
...
5. **MONITOR** dev completion via TaskCompleted events:
   ...
   - Fallbabs: Check TaskList if no events received in 60s
...
7. **DOCS SYNC EVAL** (MANDATORY for cook -- from /bs:cook finalize):
   ```
...
## ON `/bs:team review <scope>` [--reviewers N]:

*Wraps /bs:code-review skill -- scout, review, synthesize with evidence gates.*
...
## ON `/bs:team debug <issue>` [--debuggers N]:

*Wraps /bs:fix skill -- root-cause-first, adversarial hypotheses, disprove to converge.*
...
5. **MONITOR** via TaskCompleted events. Debuggers should message each other -- let them converge.
   ...
   - Fallbabs: Check TaskList if no events in 60s
