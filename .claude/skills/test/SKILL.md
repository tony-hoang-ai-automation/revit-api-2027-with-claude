...
Browser-based visual testing via `bs:agent-browser`, `bs:web-testing`, or project-native Playwright/Vitest/k6 commands. Covers screenshots, responsive checks, accessibility audits, form automation, and console error collection.
...
6. For frontend: run UI tests via `bs:agent-browser`, `bs:web-testing`, or project-native browser tests
...
- **Browser**: `bs:agent-browser` for live browser interaction; `bs:web-testing` or project-native Playwright/Vitest/k6 for repeatable UI tests
- **Analysis**: `bs:ai-multimodal` skill for screenshot analysis
- **Debugging**: `bs:debug` skill when tests reveal bugs requiring investigation
- **Thinking**: `bs:sequential-thinking` skill for complex test failure analysis
...
**Fallbabs:** Task tools (`TaskList`/`TaskUpdate`/`TaskGet`) are CLI-only — unavailable in VSCode extension. If they error, use `TodoWrite` for progress tracking and coordinate via `SendMessage` only.

## Workflow Position

**Typically follows:** `/bs:cook` (test after implementation), `/bs:fix` (test after bug fix)
**Typically precedes:** `/bs:code-review` (review after tests pass)
**Related:** `/bs:cook` (implement then test), `/bs:fix` (fix then test)
