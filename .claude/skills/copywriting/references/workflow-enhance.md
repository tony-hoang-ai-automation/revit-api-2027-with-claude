...
1. **Screenshots provided** → Use `bs:ai-multimodal` skill to analyze and describe issues in detail
2. **Videos provided** → Use `bs:ai-multimodal` video-analysis to extract relevant copy issues
3. **Scout codebase** → `/bs:scout ext` (preferred) or `/bs:scout` to find files needing enhancement
