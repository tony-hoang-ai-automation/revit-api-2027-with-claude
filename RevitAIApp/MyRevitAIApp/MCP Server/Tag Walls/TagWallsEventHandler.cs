...
namespace MyRevitAIApp.McpServer.TagWalls
{
...
try
                        {
                            ...
                            if (locationCurve != null)
                            {
                                ...
                                if (tag != null)
                                {
                                    createdTags.Add(new
                                    {
                                        id = tag.Id.GetValue().ToString(),
                                        wallId = wall.Id.GetValue().ToString(),
                        ...
                        {
                            errors.Add($"标记墙体 {wall.Id.GetValue()} 出错: {ex.Message}");
                        }
#endif
                    }
        ...
        private FamilySymbol FindWallTagType(Document doc)
        {
            ...
            if (!string.IsNullOrEmpty(_tagTypeId))
            {
                if (int.TryParse(_tagTypeId, out int id))
                {
                    ...
                    if (element != null && element is FamilySymbol symbol &&
                        (symbol.Category.Id.GetValue() == (int)BuiltInCategory.OST_WallTags ||
                         symbol.Category.Id.GetValue() == (int)BuiltInCategory.OST_MultiCategoryTags))
                    {
                        return symbol;
                    }
                }
            ...
            FamilySymbol wallTagType = tagCollector.OfClass(typeof(FamilySymbol))
                                                  .WhereElementIsElementType()
                                                  .Where(e => e.Category != null &&
                                                         e.Category.Id.GetValue() == (int)BuiltInCategory.OST_WallTags)
                                                  .Cast<FamilySymbol>()
                                                  .FirstOrDefault();
            ...
            if (wallTagType == null)
            {
                wallTagType = tagCollector.OfClass(typeof(FamilySymbol))
                                         .WhereElementIsElementType()
                                         .Where(e => e.Category != null &&
                                                e.Category.Id.GetValue() == (int)BuiltInCategory.OST_MultiCategoryTags)
                                         .Cast<FamilySymbol>()
                                         .FirstOrDefault();
            }
